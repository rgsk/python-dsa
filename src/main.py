...
def setup(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        correct_output_file_path = os.path.join(
            script_directory, 'correct_output.txt')
                ...
                try:
                    ...
                    if out_norm == cor_norm:
                        ...
                        for i in range(max_lines):
                            o = out_lines[i] if i < len(
                                out_lines) else "<missing>"
                            c = cor_lines[i] if i < len(
                                cor_lines) else "<missing>"
                            if o != c:
                                print(f"  Line {i+1}:")
                                print(f"    output : {o}")
                                print(f"    correct: {c}")
